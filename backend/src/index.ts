    ...
    origin:
      allowed === "*"
        ...
        : (origin, callback) => {
            ...
            return callback(Object.assign(new Error("Not allowed by CORS"), { status: 403 }) as any);
          },
...
const upload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    ...
    return cb(Object.assign(new Error("Invalid file type"), { status: 400 }) as any);
  }
});
...
app.get("/api/environment", async (req, res, next) => {
  try {
    ...
    if (!Number.isFinite(lat) || !Number.isFinite(lon)) return res.status(400).json({ error: "lat and lon are required numbers" });
    if (lat < -90 || lat > 90 || lon < -180 || lon > 180) return res.status(400).json({ error: "lat/lon out of range" });
...
app.post("/api/identify", (upload.single("image") as unknown as express.RequestHandler), async (req, res, next) => {
  const file = req.file;
  if (!file) return res.status(400).json({ error: "image file is required (multipart/form-data, field: image)" });
  try {
...
app.get("/api/plants", async (req, res, next) => {
  try {
    const { query, tags, space } = req.query as { query?: string; tags?: string; space?: "indoor" | "balcony" | "outdoor" | string };
    if (query && query.length > 64) return res.status(400).json({ error: "query too long" });
    ...
    const q: any = {};
    if (query && query.trim().length) q.$text = { $search: query.trim() };
    if (tags) q.tags = { $in: tags.split(",").map(s => s.trim()).filter(Boolean).slice(0, 10) };
    if (space) q.spaces = { $in: [space] };
...
app.get("/api/recommendations", async (req, res, next) => {
  try {
    ...
    const allowedSpaces = new Set<"indoor" | "balcony" | "outdoor">(["indoor", "balcony", "outdoor"]);
    const space = allowedSpaces.has(rawSpace as any) ? (rawSpace as "indoor" | "balcony" | "outdoor") : "indoor";
    ...
    const scored = plants
      .map(p => {
        ...
        // Temperature fit
        if (temp >= p.minTempC && temp <= p.maxTempC) score += 3;
        else if (Math.abs(temp - (temp < p.minTempC ? p.minTempC : p.maxTempC)) <= 3) score += 1;

        // Sunlight fit (cloudiness proxy)
        const sunlight = cloud <= 30 ? "full" : cloud <= 70 ? "partial" : "shade";
        if (p.sunlight === sunlight) score += 2;

        // Air quality preference
        const aqi = air.aqiUS ?? 50;
        if (aqi > 80 && p.airPurifying) score += 2;

        // Preferences tags
        for (const pref of preferences) {
          if (pref === "air_purifying" && p.airPurifying) score += 1;
          if (pref === "pet_friendly" && p.petFriendly) score += 1;
          if (pref && p.tags.includes(pref)) score += 1;
        }
