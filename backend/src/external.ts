...
export async function fetchSoilMoisture(lat: number, lon: number) {
  if (!env.NASA_API_KEY) return { soilMoisture: null, source: null };
  try {
...
export async function identifyPlantWithPlantNet(imagePath: string) {
  try {
