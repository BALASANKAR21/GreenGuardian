...
app.post(
  '/api/users/profile',
  authenticate,
  validateBody(['displayName', 'email']),
...
// Plant routes
app.get('/api/plants/search', authenticate, async (req: AuthRequest, res, next) => {
  try {
    ...
    const plants = await services.plants.searchPlants(
      query,
...
app.get('/api/plants/:id', authenticate, async (req: AuthRequest, res, next) => {
  try {
...
// Garden routes
app.get('/api/gardens', authenticate, async (req: AuthRequest, res, next) => {
  try {
...
app.post(
  '/api/gardens/plants',
  authenticate,
  validateBody(['plantId', 'location']),
  async (req: AuthRequest, res, next) => {
    try {
      const { plantId, location, notes } = req.body;
      const success = await services.gardens.addPlantToGarden(
        req.user!.uid,
        plantId,
        location,
        notes
...
app.patch(
  '/api/gardens/plants/:plantId',
  authenticate,
  async (req: AuthRequest, res, next) => {
    try {
      const success = await services.gardens.updatePlantInGarden(
        req.user!.uid,
        req.params.plantId,
        req.body
      );
...
app.post(
  ...
  authenticate,
  validateBody(['location', 'readings']),
  async (req: AuthRequest, res, next) => {
    try {
...
app.get(
  ...
  async (req: AuthRequest, res, next) => {
    try {
      ...
      const data = await services.environmentalData.getEnvironmentalData(
        req.user!.uid,
